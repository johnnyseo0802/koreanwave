...
export async function moderateReview(id: unknown, decision: unknown): Promise<{ ok: boolean; message: string }> {
  if (typeof id !== "string" || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(id)
    || (decision !== "approved" && decision !== "rejected")) {
    return { ok: false, message: "Please select a valid review and decision." };
  }
  ...
  try {
    ...
    if (!data) return { ok: false, message: "This review is no longer available for moderation. Please refresh the queue." };
    return { ok: true, message: decision === "approved" ? "Review approved." : "Review rejected." };
  } catch {
